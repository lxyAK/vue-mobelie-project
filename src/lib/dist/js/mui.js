...
(function ($) {
  ...
  var delegateFn = function (element, event, selector, callback) {
    return function (e) {
      ...
      for (; target && target !== document; target = target.parentNode) {
        ...
        if (
          ~['click', 'tap', 'doubletap', 'longtap', 'hold'].indexOf(event) &&
        ...
        $.each(
          ...
          function (selector, callbacks) {
            // same selector
            selectorAlls[selector] ||
              (selectorAlls[selector] = $.qsa(selector, element))
            if (
              selectorAlls[selector] &&
              ~selectorAlls[selector].indexOf(target)
            ) {
              if (!matches[selector]) {
  ...
  $.fn.on = function (event, selector, callback) {
    ...
    return this.each(function () {
      ...
      var delegateEvents =
        delegates[element._mid] || (delegates[element._mid] = {})
      var delegateCallbackObjs =
        delegateEvents[event] || (delegateEvents[event] = {})
      if ($.isEmptyObject(delegateCallbackObjs)) {
        isAddEventListener = true
      }
      var delegateCallbacks =
        delegateCallbackObjs[selector] || (delegateCallbackObjs[selector] = [])
  ...
  $.fn.off = function (event, selector, callback) {
    return this.each(function () {
      ...
      if (!event) {
        ...
        $.each(
          delegateCallbacks,
          function (index, delegateCallback) {
      ...
      if (delegates[_mid]) {
        ...
        if (
          !delegates[_mid][event] ||
          $.isEmptyObject(delegates[_mid][event])
        ) {
          findDelegateFn(this, event).forEach(
            function (fn) {
    ...
    for (; target && target !== document; target = target.parentNode) {
      ...
      $.each($.targetHandles, function (index, targetHandle) {
        ...
        if (
          !isFound &&
          !founds[name] &&
          targetHandle.hasOwnProperty('handle')
        ) {
    ...
    document.addEventListener('focusin', function (e) {
      ...
      if (
        ...
        for (; target && target !== document; target = target.parentNode) {
          ...
          if (
            (classList && classList.contains(CLASS_BAR_TAB)) ||
            classList.contains(CLASS_BAR_FOOTER) ||
...
(function ($, window) {
  ...
  var getScale = function (starts, moves) {
    if (starts.length >= 2 && moves.length >= 2) {
      ...
      return (
        getDistance(moves[1], moves[0], props) /
  ...
  var getRotation = function (start, end) {
    var props = ['pageX', 'pageY']
    return (
      getAngle(end[1], end[0], props) - getAngle(start[1], start[0], props)
    )
  }
  /**
   * px per ms
  ...
  var calDelta = function (touch) {
    ...
    if (
      touch.gesture.type === $.EVENT_START ||
      touch.gesture.type === $.EVENT_END
    ) {
  ...
  var calTouchData = function (touch) {
    ...
    var offsetCenter = firstMultiTouch
      ? firstMultiTouch.center
      : firstTouch.center
    ...
    touch.scale = firstMultiTouch
      ? getScale(firstMultiTouch.touches, touches)
      : 1
    touch.rotation = firstMultiTouch
      ? getRotation(firstMultiTouch.touches, touches)
      : 0
  ...
  var calIntervalTouchData = function (touch) {
    ...
    if (
      ...
      (deltaTime > CAL_INTERVAL || last.velocity === undefined)
    ) {
  ...
  var getTouches = function (event, touch) {
    ...
    if (
      (type === $.EVENT_START || type === $.EVENT_MOVE) &&
      allTouches.length === 1
    ) {
    ...
    touch.isFinal =
      (type === $.EVENT_END || type === $.EVENT_CANCEL) &&
  ...
  window.addEventListener(
    ...
    function (e) {
      ...
      if (
        ($.os.android || $.os.ios) &&
        (($.targets.popover && e.target === $.targets.popover) ||
          $.targets.tab ||
          $.targets.offcanvas ||
          $.targets.modal)
      ) {
  ...
  var handle = function (event, touch) {
    ...
    switch (event.type) {
      ...
      case $.EVENT_CANCEL:
        ...
        if (
          session.flickStart &&
          options.flickMaxTime > now - flickStartTime &&
...
(function ($, name) {
  var handle = function (event, touch) {
    ...
    if (event.type === $.EVENT_END || event.type === $.EVENT_CANCEL) {
      ...
      if (
        touch.direction &&
        options.swipeMaxTime > touch.deltaTime &&
    ...
    switch (event.type) {
      ...
      case $.EVENT_MOVE:
        ...
        if (session.lockDirection && session.startDirection) {
          if (
            session.startDirection &&
            session.startDirection !== touch.direction
          ) {
            if (
              session.startDirection === 'up' ||
              session.startDirection === 'down'
            ) {
    ...
    switch (event.type) {
      case $.EVENT_END:
        ...
        if (
          !target ||
          target.disabled ||
            (target.classList && target.classList.contains('mui-disabled'))
        ) {
          return
        }
        if (
          touch.distance < options.tapMaxDistance &&
          touch.deltaTime < options.tapMaxTime
        ) {
          if (
            $.options.gestureConfig.doubletap &&
            lastTarget &&
            lastTarget === target
          ) {
            // same target
            if (
              lastTapTime &&
              touch.timestamp - lastTapTime < options.tapMaxInterval
            ) {
    ...
    switch (event.type) {
      ...
      case $.EVENT_MOVE:
        if ($.options.gestureConfig.pinch) {
          ...
          var lastScale =
            typeof touch.lastScale === 'undefined' ? 1 : touch.lastScale
...
(function ($) {
  ...
  function handleScroll (wrapper, iframe) {
    var key =
      'MUI_SCROLL_POSITION_' + document.location.href + '_' + iframe.src
      ...
      if (nShow.autoShow) {
        var showWebview = function () {
          ...
          options.afterShowMethodName &&
            webview.evalJS(
              options.afterShowMethodName + "('" + JSON.stringify(params) + "')"
            )
        }
  ...
  $.openWindowWithTitle = function (options, titleConfig) {
    ...
    if (webview) {
      ...
      if (titleConfig) {
        ...
        var tid = $.options.titleConfig.id
          ? $.options.titleConfig.id
          : id + '_title'
        ...
        if (_backImage.base64Data || _backImage.imgSrc) {
          ...
          backClick = {
            ...
            right:
              parseInt(_backImage.position.left) +
              parseInt(_backImage.position.width)
          }
  ...
  if ($.os.plus && $.os.android) {
    $.addBack({
      ...
      handle: function () {
        ...
        if (
          $.targets._popover &&
          $.targets._popover.classList.contains('mui-active')
        ) {
        ...
        var previewImage =
          $.isFunction($.getPreviewImage) && $.getPreviewImage()
...
(function ($, window, undefined) {
  ...
  $.ajax = function (url, options) {
    ...
    if (
      ...
      ((!options || options.cache !== true) && dataType === 'script')
    ) {
...
(function ($, document, undefined) {
  ...
  var CLASS_LOADING_UP =
    CLASS_PULL_LOADING + ' ' + CLASS_ICON + ' ' + CLASS_ICON_PULLDOWN
  var CLASS_LOADING_DOWN =
    CLASS_PULL_LOADING + ' ' + CLASS_ICON + ' ' + CLASS_ICON_PULLDOWN
  var CLASS_LOADING =
    CLASS_PULL_LOADING + ' ' + CLASS_ICON + ' ' + CLASS_SPINNER
  ...
  var PullRefresh = {
    ...
    _initPocket: function () {
      ...
      if (options.down && options.down.hasOwnProperty('callback')) {
        ...
        if (!this.topPocket) {
          this.topPocket = this._createPocket(
            CLASS_PULL_TOP_POCKET,
            options.down,
            CLASS_LOADING_DOWN
          )
      ...
      if (options.up && options.up.hasOwnProperty('callback')) {
        ...
        if (!this.bottomPocket) {
          this.bottomPocket = this._createPocket(
            CLASS_PULL_BOTTOM_POCKET,
            options.up,
            CLASS_LOADING
          )
      ...
      if (!isPulldown) {
        switch (title) {
          case this.options.up.contentdown:
            caption.className =
              CLASS_PULL_CAPTION + ' ' + CLASS_PULL_CAPTION_DOWN
            break
          case this.options.up.contentrefresh:
            caption.className =
              CLASS_PULL_CAPTION + ' ' + CLASS_PULL_CAPTION_REFRESH
            break
          case this.options.up.contentnomore:
            caption.className =
              CLASS_PULL_CAPTION + ' ' + CLASS_PULL_CAPTION_NOMORE
            break
        }
      }
    },
...
(function ($, window, document, undefined) {
  ...
  var Scroll = $.Class.extend({
    ...
    _getSnapX: function (offsetLeft) {
      return Math.max(
        Math.min(0, -offsetLeft + this.wrapperWidth / 2),
        this.maxScrollX
      )
    },
    ...
    _initEvent: function (detach) {
      ...
      if (segmentedControl) {
        ...
        mui(segmentedControl)[detach ? 'off' : 'on'](
          'click',
          'a',
          $.preventDefault
        )
      ...
      if (this.parallaxElement && this.options.scrollY) {
        ...
        if (scale > 1) {
          this.parallaxImgStyle.opacity =
            1 - (parallaxY / 100) * this.options.parallaxRatio
          this.parallaxStyle.webkitTransform =
            this._getTranslateStr(0, -parallaxY) +
            ' scale(' +
            scale +
            ',' +
            scale +
    ...
    reLayout: function () {
      ...
      var paddingLeft =
        parseFloat($.getStyles(this.wrapper, 'padding-left')) || 0
      var paddingRight =
        parseFloat($.getStyles(this.wrapper, 'padding-right')) || 0
      var paddingTop =
        parseFloat($.getStyles(this.wrapper, 'padding-top')) || 0
      var paddingBottom =
        parseFloat($.getStyles(this.wrapper, 'padding-bottom')) || 0
  ...
  var Indicator = function (scroller, options) {
    ...
    if (this.options.fade) {
      ...
      this.wrapperStyle.webkitTransitionDuration =
        this.options.fixedBadAndorid && $.os.isBadAndroid ? '0.001s' : '0ms'
    ...
    $.extend(
      {
        ...
        disablePullupToRefresh: function () {
          ...
          this.bottomPocket.className =
            'mui-pull-bottom-pocket' + ' ' + CLASS_HIDDEN
...
(function ($, window) {
  ...
  var Slider = ($.Slider = $.Scroll.extend({
    ...
    _handleSlide: function (e) {
      ...
      if (indicatorWrap) {
        if (indicatorWrap.getAttribute('data-scroll')) {
          ...
          $(indicatorWrap)
            .scroll()
            .gotoPage(detail.slideNumber)
        }
        var indicators = indicatorWrap.querySelectorAll('.mui-indicator')
        if (indicators.length > 0) {
          // 图片轮播
          for (var i = 0, len = indicators.length; i < len; i++) {
            indicators[i].classList[
              i === detail.slideNumber ? 'add' : 'remove'
            ](CLASS_ACTIVE)
          }
        } else {
    ...
    _handleIndicatorTap: function (event) {
      ...
      var target = event.target
      if (
        target.classList.contains(CLASS_ACTION_PREVIOUS) ||
      ...
      if (e.type === 'flick') {
        if (detail.deltaTime < 200) {
          ...
          this.x = this._getPage(
            this.slideNumber + (direction === 'right' ? -1 : 1),
            true
          ).x
        }
        this.resetPosition(this.options.bounceTime)
    ...
    _initSnap: function () {
      ...
      if (!this.currentPage.x) {
        ...
        this.lastSlideNumber =
          typeof this.lastSlideNumber === 'undefined'
            ? 0
            : this.lastSlideNumber
      } else {
    ...
    setTranslate: function (x, y) {
      ...
      if (progressBar) {
        this.progressBarStyle.webkitTransform = this._getTranslateStr(
          -x * (this.progressBarWidth / this.wrapperWidth),
          0
        )
      }
    },
...
(function ($, document) {
  ...
  $.plusReady(function () {
      ...
      $.extend(
        {
          ...
          disablePulldownToRefresh: function () {
            var webview = plus.webview.currentWebview()
            if (
              this.options.down.style &&
              this.options.down.style == 'circle'
            ) {
              // 单webview模式禁止原生下拉刷新
    ...
    $.fn.pullRefresh_native = function (options) {
      ...
      var pullRefreshApi = null
      var attrWebviewId =
        options.webviewId && options.webviewId.replace(/\//g, '_') // 替换所有"/"
  ...
  var OffCanvas = $.Class.extend({
        ...
        case 'dragend':
          if (this.isDragging) {
            ...
            if (!this.slideIn) {
              if (x >= 0) {
                ratio =
                  (this.offCanvasLeftWidth && x / this.offCanvasLeftWidth) || 0
              } else {
                ratio =
                  (this.offCanvasRightWidth && x / this.offCanvasRightWidth) ||
                  0
              }
              if (ratio === 0) {
    ...
    updateTranslate: function (x) {
      if (x !== this.lastTranslateX) {
        if (!this.slideIn) {
          if (
            (!this.offCanvasLeft && x > 0) ||
            (!this.offCanvasRight && x < 0)
          ) {
    ...
    setTranslateX: $.animationFrame(function (x) {
      if (this.scroller) {
        if (this.scalable && this.offCanvas.parentNode === this.wrapper) {
          ...
          var zoomInScale =
            this.options.scale + (1 - this.options.scale) * percent
          var zoomOutOpacity = 1 - (1 - this.options.opacity) * percent
          var zoomInOpacity =
            this.options.opacity + (1 - this.options.opacity) * percent
          ...
          this.offCanvas.style.webkitTransform =
            'translate3d(0,0,0) scale(' + zoomInScale + ')'
          this.scroller.style.webkitTransform =
            'translate3d(' + x + 'px,0,0) scale(' + zoomOutScale + ')'
        } else {
          if (this.slideIn) {
            this.offCanvas.style.webkitTransform =
    ...
    isShown: function (direction) {
      ...
      if (!this.slideIn) {
        ...
        if (direction === 'left') {
          shown =
            this.classList.contains(CLASS_ACTIVE) &&
  ...
  window.addEventListener('tap', function (e) {
    ...
    for (; target && target !== document; target = target.parentNode) {
      if (
        target.tagName === 'A' &&
        target.hash &&
        target.hash === '#' + $.targets.offcanvas.id
      ) {
        e.detail && e.detail.gesture && e.detail.gesture.preventDefault() // fixed hashchange
        $($.targets._container)
          .offCanvas()
          .toggle($.targets.offcanvas)
...
(function ($, window, document, name) {
  ...
  var handle = function (event, target) {
    if (target.tagName === 'A' && target.hash) {
      ...
      if (
        $.targets._popover &&
        $.targets._popover.classList.contains(CLASS_POPOVER)
      ) {
        return target
      } else {
  ...
  var removeBackdrop = function (popover) {
    ...
    removeBackdropTimer = $.later(function () {
      if (
        !popover.classList.contains(CLASS_ACTIVE) &&
        backdrop.parentNode &&
  ...
  var togglePopover = function (popover, anchor, state) {
    if (
      (state === 'show' && popover.classList.contains(CLASS_ACTIVE)) ||
      (state === 'hide' && !popover.classList.contains(CLASS_ACTIVE))
    ) {
      return
    }
    ...
    if (
      popover.classList.contains(CLASS_BAR_POPOVER) ||
  ...
  var calPosition = function (popover, anchor, isActionSheet) {
    ...
    var diff = 0
    var arrowLeft = 0
    var defaultPadding = popover.classList.contains(CLASS_ACTION_POPOVER)
      ? 0
      : 5
    ...
    if (pHeight + arrowSize < offset.top - window.pageYOffset) {
      ...
      pHeight + arrowSize <
      wHeight - (offset.top - window.pageYOffset) - aHeight
    ) {
      // bottom
      position = 'bottom'
    ...
    if (position === 'top' || position === 'bottom') {
      ...
      if (arrow) {
        ...
        arrowLeft = Math.max(
          Math.min(arrowLeft, pWidth - arrowSize * 2 - 6),
          6
        )
...
(function ($, window, document, name, undefined) {
  ...
  var handle = function (event, target) {
    if (
      target.classList &&
      (target.classList.contains(CLASS_CONTROL_ITEM) ||
        target.classList.contains(CLASS_TAB_ITEM))
    ) {
      if (
        target.parentNode &&
        target.parentNode.classList &&
        target.parentNode.classList.contains(CLASS_SEGMENTED_CONTROL_VERTICAL)
      ) {
  ...
  window.addEventListener('tap', function (e) {
    ...
    for (
      ...
      if (segmentedControl.classList.contains(CLASS_SEGMENTED_CONTROL)) {
        activeTab = segmentedControl.querySelector(
          classSelector + '.' + CLASS_CONTROL_ITEM
        )
        break
      } else if (segmentedControl.classList.contains(CLASS_TAB_BAR)) {
        activeTab = segmentedControl.querySelector(
          classSelector + '.' + CLASS_TAB_ITEM
        )
      }
    }
    ...
    activeBodies = parentNode.querySelectorAll(
      '.' + CLASS_CONTROL_CONTENT + classSelector
    )
    for (var i = 0; i < activeBodies.length; i++) {
      var activeBody = activeBodies[i]
      activeBody.parentNode === parentNode &&
...
(function ($, window, name) {
  ...
  Toggle.prototype.setTranslateX = $.animationFrame(function (x) {
    ...
    var isChanged = false
    if (
      (this.initialState && -x > this.handleX / 2) ||
      (!this.initialState && x > this.handleX / 2)
    ) {
      isChanged = true
    }
...
(function ($, window, document) {
      ...
      if (buttonsRight && buttonsRight.length > 0) {
        ...
        if (translateX < -sliderActionRightWidth) {
          translateX =
            ...
            Math.pow(-translateX - sliderActionRightWidth, overFactor)
        }
      ...
      if (buttonsLeft && buttonsLeft.length > 0) {
        ...
        if (translateX > sliderActionLeftWidth) {
          translateX =
            ...
            Math.pow(translateX - sliderActionLeftWidth, overFactor)
        }
        for (var i = 0, len = buttonsLeft.length; i < len; i++) {
          var buttonLeft = buttonsLeft[i]
          if (typeof buttonLeft._buttonOffset === 'undefined') {
            buttonLeft._buttonOffset =
              sliderActionLeftWidth -
    ...
    for (; target && target !== document; target = target.parentNode) {
      if (target.classList) {
        ...
        if (
          ...
          classList.contains(CLASS_TOGGLE) ||
          classList.contains(CLASS_BTN) ||
        ...
        if (classList.contains(CLASS_TABLE_VIEW_CELL)) {
          ...
          if (
            !cell.parentNode.classList.contains(CLASS_RADIO_VIEW) &&
            selected &&
            selected !== cell
          ) {
  ...
  var handleEvent = {
    ...
    drag: function (event) {
      ...
      if (!isMoved) {
        ...
        if (sliderHandle) {
          ...
          if (isOpened) {
            openedActions = cell.querySelector(
              SELECTOR_SLIDER_LEFT + SELECTOR_SELECTED
            )
              ? 'left'
              : 'right'
          }
      ...
      if (isMoved) {
        ...
        if (
          (translate > 0 && !buttonsLeft) ||
          (translate < 0 && !buttonsRight)
        ) {
          if (!isOpened) {
            return
          }
    ...
    dragend: function (event) {
      ...
      if (action === 'open') {
        var newTranslate =
          sliderDirection === 'toLeft' ? -actionsWidth : actionsWidth
  ...
  window.addEventListener('tap', function (event) {
    ...
    if (
      classList.contains('mui-collapse') &&
...
(function ($, window) {
  ...
  $.toast = function (message, options) {
    ...
    if ($.os.plus && options.type !== 'div') {
      ...
      toast.innerHTML =
        '<div class="' + 'mui-toast-message' + '">' + message + '</div>'
...
(function ($, window, document) {
  ...
  var createButtons = function (btnArray) {
    ...
    for (var i = 0; i < length; i++) {
      btns.push(
        '<span class="' +
          CLASS_POPUP_BUTTON +
          (i === length - 1 ? ' ' + CLASS_POPUP_BUTTON_BOLD : '') +
          '">' +
          btnArray[i] +
          '</span>'
      )
    ...
    return (
      '<div class="' + CLASS_POPUP_BUTTONS + '">' + btns.join('') + '</div>'
    )
  }
  ...
  var createAlert = function (message, title, btnValue, callback, type) {
    ...
    if (!$.os.plus || type === 'div') {
      return createPopup(
        createInner(message, title || '提示') +
          createButtons([btnValue || '确定']),
        callback
      )
    }
    return plus.nativeUI.alert(
      message,
      callback,
      title || '提示',
      btnValue || '确定'
  ...
  var createPrompt = function (
    ...
    if (!$.os.plus || type === 'div') {
      return createPopup(
        createInner(message, title || '提示', createInput(placeholder)) +
          createButtons(btnArray || ['取消', '确认']),
        callback
      )
    }
    return plus.nativeUI.prompt(
      message,
      callback,
      title || '提示',
      placeholder,
...
(function ($, document) {
  ...
  var showProgressbar = function (container, progress, color) {
    ...
    if (container.classList.contains(CLASS_PROGRESSBAR)) {
      ...
      var progressbars = container.querySelectorAll(
        SELECTOR_PROGRESSBAR + ':not(.' + CLASS_PROGRESSBAR_OUT + ')'
      )
      if (progressbars) {
        for (var i = 0, len = progressbars.length; i < len; i++) {
      ...
      if (!progressbar) {
        ...
        progressbar.className =
          CLASS_PROGRESSBAR +
          ' ' +
          CLASS_PROGRESSBAR_IN +
          (typeof progress !== 'undefined'
            ? ''
            : ' ' + CLASS_PROGRESSBAR_INFINITE) +
          (color ? ' ' + CLASS_PROGRESSBAR + '-' + color : '')
  ...
  var hideProgressbar = function (container) {
    ...
    if (
      !classList.contains(CLASS_PROGRESSBAR_IN) ||
      classList.contains(CLASS_PROGRESSBAR_OUT)
    ) {
      return
    }
  ...
  var setProgressbar = function (container, progress, speed) {
    ...
    if (
      !progressbar ||
      progressbar.classList.contains(CLASS_PROGRESSBAR_INFINITE)
    ) {
      return
    }
...
(function ($, window, document) {
  ...
  var Input = function (element, options) {
    ...
    if (~this.options.actions.indexOf('slider')) {
      ...
      if (~this.options.actions.indexOf('clear')) {
        this.clearActionClass =
          CLASS_ICON + ' ' + CLASS_ICON_CLEAR + ' ' + CLASS_HIDDEN
  ...
  Input.prototype.initAction = function () {
    ...
    if (row) {
      if (self.sliderActionClass) {
        self.sliderAction = self.createAction(
          row,
          self.sliderActionClass,
          self.sliderActionSelector
        )
      } else {
        if (self.searchActionClass) {
          self.searchAction = self.createAction(
            row,
            self.searchActionClass,
        ...
        if (self.speechActionClass) {
          self.speechAction = self.createAction(
            row,
            self.speechActionClass,
        ...
        if (self.clearActionClass) {
          self.clearAction = self.createAction(
            row,
            self.clearActionClass,
        ...
        if (self.passwordActionClass) {
          self.passwordAction = self.createAction(
            row,
            self.passwordActionClass,
  ...
  Input.prototype.initElementEvent = function () {
    ...
    if (this.sliderActionClass) {
      ...
      var showTip = function () {
        ...
        var scaleWidth =
          (width / distince) * Math.abs(element.value - element.min)
        tooltip.style.left =
          14 + offsetLeft + scaleWidth - tooltipWidth / 2 + 'px'
      ...
      if (this.clearActionClass) {
        ...
        $.each(['keyup', 'change', 'input', 'focus', 'cut', 'paste'], function (
          index,
          type
        ) {
          (function (type) {
  ...
  Input.prototype.setPlaceholder = function (text) {
    if (this.searchActionClass) {
      ...
      placeholder &&
        (placeholder.getElementsByTagName('span')[1].innerText = text)
    } else {
  ...
  $.fn.input = function (options) {
    ...
    this.each(function () {
      ...
      if (this.type === 'range' && row.classList.contains('mui-input-range')) {
        ...
        if (
          !($.os.android && $.os.stream) &&
          classList.contains('mui-input-speech')
        ) {
          actions.push('speech')
        }
...
(function ($, window) {
  ...
  Transparent.prototype.handleScroll = function (e) {
    ...
    this._style.backgroundColor =
      'rgba(' + this._R + ',' + this._G + ',' + this._B + ',' + opacity + ')'
    if (opacity > this._A) {
  ...
  $.fn.transparent = function (options) {
    ...
    this.each(function () {
      ...
      if (!transparentApi) {
        ...
        transparentApi = this.mui_plugin_transparent = new Transparent(
          this,
          options
        )
      }
    ...
    checkValue: function () {
      ...
      if (val == null || val == '' || isNaN(val)) {
        ...
        if (
          ...
          !isNaN(self.options.max) &&
          val >= parseInt(self.options.max)
        ) {
        ...
        if (
          ...
          !isNaN(self.options.min) &&
          val <= parseInt(self.options.min)
        ) {
